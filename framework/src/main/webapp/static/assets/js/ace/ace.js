...
(function($, undefined) {
	...
	ace.vars['touch'] = ('ontouchstart' in document.documentElement);// (('ontouchstart'
																		// in
...
jQuery(function($) {
		...
		if ($.fn.ace_sidebar_scroll)
			$sidebar.ace_sidebar_scroll({
				...
				'smooth_scroll' : 150, // > 0 means smooth_scroll, time in ms,
										// used in first approach only, better
										// to be almost half the amount of
										// submenu transition time
				'outside' : false
					// true && ace.vars['touch'] //used in first approach only,
					// true means the scrollbars should be outside of the
					// sidebar
				});
		...
		if ($.fn.ace_ajax) {
			$('[data-ajax-content=true]').ace_ajax({
				...
				'content_url' : function(hash) {
					// ***NOTE***
					// this is for Ace demo only, you should change it to return
					// a valid URL
	...
	function dropdownAutoPos() {
		...
		$(document).on('click.dropdown.pos',
				'.dropdown-toggle[data-position="auto"]', function() {
					var offset = $(this).offset();
					var parent = $(this.parentNode);

					if (parseInt(offset.top + $(this).height()) + 50 > (ace.helper
							.scrollTop()
							+ ace.helper.winHeight() - parent
							.find('.dropdown-menu').eq(0).height()))
	...
	function navbarHelpers() {
		...
		$('.ace-nav [class*="icon-animated-"]').closest('a').one('click',
				function() {
					var icon = $(this).find('[class*="icon-animated-"]').eq(0);
					var $match = icon.attr('class')
				...
				.each(function() {
					...
					$(this).tooltip({
						'placement' : function(context, source) {
							...
							if (parseInt(offset.left) < parseInt(document.body.scrollWidth
									/ 2))
								return 'right';
							return 'left';
						},
		...
		if (scroll_btn.length > 0) {
			...
			$(window).on('scroll.scroll_btn', function() {
				...
				if (scroll > parseInt(h / 4)
						|| (scroll > 0 && body_sH >= h && h + scroll >= body_sH
								- 1)) {// || for smaller pages, when reached
										// end of page
					if (!is_visible) {
			...
			scroll_btn.on(ace.click_event, function() {
						var duration = Math.min(500, Math.max(100,
										parseInt(ace.helper.scrollTop() / 3)));
						$('html,body').animate({
									scrollTop : 0
								}, duration);
						return false;
	...
	function someBrowserFix() {
		...
		if (ace.vars['ios_safari']) {
			$(document).on('ace.settings.ios_fix',
					function(e, event_name, event_val) {
						...
						if (event_val == true) {
							$(document).on('focus.ios_fix',
									'input,textarea,.wysiwyg-editor',
									function() {
										$(window).on('scroll.ios_fix',
												function() {
													var navbar = $('#navbar')
															.get(0);
													if (navbar)
		...
		$(document).on('hide.bs.collapse show.bs.collapse', function(ev) {
			...
			panel.find(ace.vars['.icon']).each(function() {
				...
				if ($icon_down) {
					...
					return false;// ignore other icons that match, one is
									// enough
				}
	...
	function smallDeviceDropdowns() {
		...
		function adjustNavbarDropdown() {
			...
			if ($sub.css('position') == 'fixed') {
				...
				var offset_w = win_width > 320 ? 60 : (win_width > 240
						? 40
						: 30);
				...
				var height = parseInt(Math.min(avail_height, 480, scrollHeight
								+ extra_parts));
				var left = parseInt(Math.abs((avail_width + offset_w - width)
						/ 2));
